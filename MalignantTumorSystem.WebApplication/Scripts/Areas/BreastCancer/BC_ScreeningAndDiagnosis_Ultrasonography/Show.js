$(function () {
    $.ajax({
        type:"post",
        dataType:"json",
        url: "/BreastCancer/BC_ScreeningAndDiagnosis_Ultrasonography/Show",
        data:{id:id},
        success: function (data) { 
            ...
            //--------------------------------------------------特有字段------------------------
            $("#syzqt").val(data[0].syzqt); 
            if (data[0].syz != "" && data[0].syz != null) {
                var strs = new Array();
                strs = data[0].syz.split(',');
                for (var i = 0; i < strs.length; i++) {
                    $("input:checkbox[name='syz'][value=" + strs[i] + "]").attr("checked", true);
                }
            }

            $("#jcbw").val(data[0].jcbw);
            $("#lcbx").val(data[0].lcbx);
            $("#csbx").val(data[0].csbx);
            $("#jczd").val(data[0].jczd);
            $("#ysjy").val(data[0].ysjy);
