...
{
    public class BC_ScreeningAndDiagnosis_MammographyController : BaseTopController
    {
        ...
        [Inject]
        public IMT_BC_MammographyService mT_BC_MammographyService { get; set; }
        ...
        public ActionResult List()
        {
            ...
            var list = mT_BC_MammographyService.LoadSearchEntities(commonParam);
            totalCount = commonParam.TotalCount;
            int PageCount = Convert.ToInt32(Math.Ceiling((double)totalCount / pageSize));

            List<MT_BC_Mammography> result = new List<MT_BC_Mammography>();
            result.AddRange(list);
            PagerInfo pager = new PagerInfo();
            pager.PageIndex = pageIndex;
            pager.PageSize = pageSize;
            pager.TotalCount = totalCount;
            PagerQuery<PagerInfo, List<MT_BC_Mammography>> query = new PagerQuery<PagerInfo, List<MT_BC_Mammography>>(pager, result);
        ...
        public ActionResult AddAndUpdate()
        {
            ...
            MT_BC_Mammography entity = new MT_BC_Mammography();
            ...
            //------------------------------------特有字段赋值-------------------------------------------------------
            entity.xxtz = Request["xxtz"];
            entity.zkdxz = Request["zkdxz"];
            entity.zkdxy = Request["zkdxy"];
            entity.zkxzz = Request["zkxzz"];
            entity.zkxzy = Request["zkxzy"];
            entity.zkbyz = Request["zkbyz"];
            entity.zkbyy = Request["zkbyy"];
            entity.zkmdz = Request["zkmdz"];
            entity.zkmdy = Request["zkmdy"];
            entity.ghdxz = Request["ghdxz"];
            entity.ghdxy = Request["ghdxy"];
            entity.ghsmz = Request["ghsmz"];
            entity.ghsmy = Request["ghsmy"];
            entity.ghxtz = Request["ghxtz"];
            entity.ghxty = Request["ghxty"];
            entity.ghfbz = Request["ghfbz"];
            entity.ghfby = Request["ghfby"];
            entity.jgnq = Request["jgnq"];
            entity.bdczmy = Request["bdczmy"];
            entity.qthbzx = Request["qthbzx"]; 
            ...
            if (string.IsNullOrEmpty(id))
            {
                if (mT_BC_MammographyService.AddEntity(entity))
                {
                    ...
                    ehr.item_type = Model.Enum.EHRAbstractTypeEnum.Mammography.ToString();
            ...
            {
                if (mT_BC_MammographyService.UpdateEntity(entity))
                {
                    msg = "修改成功";
                }
                else
        ...
        //查看页
         public ActionResult MammographyShow()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
        ...
        //修改页
        public ActionResult MammographyUpdate()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
        ...
        public ActionResult Show()
        {
            ...
            if (id != "")
            {
                var result = mT_BC_MammographyService.LoadEntityAsNoTracking(t => t.id.Contains(id));
