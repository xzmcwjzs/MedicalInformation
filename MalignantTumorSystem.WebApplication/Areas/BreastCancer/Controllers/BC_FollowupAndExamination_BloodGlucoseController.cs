...
{
    public class BC_FollowupAndExamination_BloodGlucoseController : BaseTopController
    {
        //
        // GET: /BreastCancer/BC_FollowupAndExamination_BloodGlucose/
        [Inject]
        ...
        [Inject]
        public IChronic_disease_BloodGlucoseService disease_BloodGlucoseService { get; set; }
        [Inject]
        public IChronic_disease_BloodGlucose_AddService disease_BloodGlucose_AddService { get; set; }
        ...
        public ActionResult List()
        {
            ...
            var disease_Comm_Testing_BloodList = disease_BloodGlucoseService.LoadSearchEntities(commonParam);
            totalCount = commonParam.TotalCount;
            int PageCount = Convert.ToInt32(Math.Ceiling((double)totalCount / pageSize));

            List<Chronic_disease_BloodGlucose> result = new List<Chronic_disease_BloodGlucose>();
            result.AddRange(disease_Comm_Testing_BloodList);
            PagerInfo pager = new PagerInfo();
            pager.PageIndex = pageIndex;
            pager.PageSize = pageSize;
            pager.TotalCount = totalCount;
            PagerQuery<PagerInfo, List<Chronic_disease_BloodGlucose>> query = new PagerQuery<PagerInfo, List<Chronic_disease_BloodGlucose>>(pager, result);
        ...
        public ActionResult ShowText2()
        {
            ...
            if (id_card_number != "")
            {
                var result = disease_BloodGlucose_AddService.GetListByNum(id_card_number, out startD, out endD);
        ...
        public ActionResult ShowText()
        {
            ...
            if (id != "")
            {
                var result = disease_BloodGlucose_AddService.GetListById(id, out startD, out endD);
                return Json(new { start = startD, end = endD, res = result }, JsonRequestBehavior.AllowGet);
            }
            else
            {
        ...
        public ActionResult ShowAllText()
        {
            ...
            if (id != "")
            {
                var result = disease_BloodGlucose_AddService.GetYearMonth(id, year, month);
