...
{
    public class BC_Treatment_EndocrinotherapyController : BaseTopController
    {
        ...
        [Inject]
        public IMT_BC_EndocrinotherapyService mT_BC_EndocrinotherapyService { get; set; }
        ...
        public ActionResult List()
        {
            ...
            var list = mT_BC_EndocrinotherapyService.LoadSearchEntities(commonParam);
            totalCount = commonParam.TotalCount;
            int PageCount = Convert.ToInt32(Math.Ceiling((double)totalCount / pageSize));

            List<MT_BC_Endocrinotherapy> result = new List<MT_BC_Endocrinotherapy>();
            result.AddRange(list);
            PagerInfo pager = new PagerInfo();
            pager.PageIndex = pageIndex;
            pager.PageSize = pageSize;
            pager.TotalCount = totalCount;
            PagerQuery<PagerInfo, List<MT_BC_Endocrinotherapy>> query = new PagerQuery<PagerInfo, List<MT_BC_Endocrinotherapy>>(pager, result);
        ...
        public ActionResult AddAndUpdate()
        {
            ...
            MT_BC_Endocrinotherapy entity = new MT_BC_Endocrinotherapy();
            ...
            //------------------------------------特有字段赋值-------------------------------------------------------
            entity.zl1q = Request["zl1q"];
            entity.zl1h = Request["zl1h"];
            entity.zl2q = Request["zl2q"];
            entity.zl2h = Request["zl2h"];
            entity.zl3q = Request["zl3q"];
            entity.zl3h = Request["zl3h"];
            ...
            if (string.IsNullOrEmpty(id))
            {
                if (mT_BC_EndocrinotherapyService.AddEntity(entity))
                {
                    ...
                    ehr.item_type = Model.Enum.EHRAbstractTypeEnum.Endocrinotherapy.ToString();
            ...
            {
                if (mT_BC_EndocrinotherapyService.UpdateEntity(entity))
                {
                    msg = "修改成功";
                }
                else
        ...
        public ActionResult EndocrinotherapyShow()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
        ...
        public ActionResult EndocrinotherapyUpdate()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
