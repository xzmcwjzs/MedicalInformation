...
{
    public class BC_ScreeningAndDiagnosis_UltrasonographyController : BaseTopController
    {
        // GET: BreastCancer/BC_ScreeningAndDiagnosis_Ultrasonography
        [Inject]
        ...
        [Inject]
        public IMT_BC_UltrasonographyService mT_BC_UltrasonographyService { get; set; }
        #region 框架页
        ...
        public ActionResult List()
        {
            ...
            var list = mT_BC_UltrasonographyService.LoadSearchEntities(commonParam);
            totalCount = commonParam.TotalCount;
            int PageCount = Convert.ToInt32(Math.Ceiling((double)totalCount / pageSize));

            List<MT_BC_Ultrasonography> result = new List<MT_BC_Ultrasonography>();
            result.AddRange(list);
            PagerInfo pager = new PagerInfo();
            pager.PageIndex = pageIndex;
            pager.PageSize = pageSize;
            pager.TotalCount = totalCount;
            PagerQuery<PagerInfo, List<MT_BC_Ultrasonography>> query = new PagerQuery<PagerInfo, List<MT_BC_Ultrasonography>>(pager, result);
        ...
        //新增页
        public ActionResult Ultrasonography()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
        ...
        public ActionResult AddAndUpdate()
        {
            ...
            MT_BC_Ultrasonography entity = new MT_BC_Ultrasonography();
            ...
            //------------------------------------特有字段赋值-------------------------------------------------------
            entity.syz = Request["syz"];
            entity.syzqt = Request["syzqt"];
            entity.jcbw = Request["jcbw"];
            entity.lcbx = Request["lcbx"];
            entity.csbx = Request["csbx"];
            entity.jczd = Request["jczd"];
            entity.ysjy = Request["ysjy"]; 
            ...
            if (string.IsNullOrEmpty(id))
            {
                if (mT_BC_UltrasonographyService.AddEntity(entity))
                {
            ...
            {
                if (mT_BC_UltrasonographyService.UpdateEntity(entity))
                {
                    msg = "修改成功";
                }
                else
        ...
        //查看
        public ActionResult UltrasonographyShow()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
        ...
        //修改页
        public ActionResult UltrasonographyUpdate()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
        ...
        public ActionResult Show()
        {
            ...
            if (id != "")
            {
                var result = mT_BC_UltrasonographyService.LoadEntityAsNoTracking(t => t.id.Contains(id));
