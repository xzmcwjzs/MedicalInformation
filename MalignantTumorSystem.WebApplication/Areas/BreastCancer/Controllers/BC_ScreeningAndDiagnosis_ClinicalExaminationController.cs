...
{
    public class BC_ScreeningAndDiagnosis_ClinicalExaminationController : BaseTopController
    {
        ...
        [Inject]
        public IMT_BC_ClinicalExaminationService mT_BC_ClinicalExaminationService { get; set; }
        ...
        public ActionResult List()
        {
            ...
            var list = mT_BC_ClinicalExaminationService.LoadSearchEntities(commonParam);
            totalCount = commonParam.TotalCount;
            int PageCount = Convert.ToInt32(Math.Ceiling((double)totalCount / pageSize));

            List<MT_BC_ClinicalExamination> result = new List<MT_BC_ClinicalExamination>();
            result.AddRange(list);
            PagerInfo pager = new PagerInfo();
            pager.PageIndex = pageIndex;
            pager.PageSize = pageSize;
            pager.TotalCount = totalCount;
            PagerQuery<PagerInfo, List<MT_BC_ClinicalExamination>> query = new PagerQuery<PagerInfo, List<MT_BC_ClinicalExamination>>(pager, result);
        ...
        //查看页
        public ActionResult ClinicalExaminationShow()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
        ...
        //修改页
        public ActionResult ClinicalExaminationUpdate()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
        ...
        public ActionResult Show()
        {
            ...
            if (id != "")
            {
                var result = mT_BC_ClinicalExaminationService.LoadEntityAsNoTracking(t => t.id.Contains(id));
