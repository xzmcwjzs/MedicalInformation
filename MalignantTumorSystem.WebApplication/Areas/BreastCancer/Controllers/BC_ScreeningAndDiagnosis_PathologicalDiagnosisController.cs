...
{
    public class BC_ScreeningAndDiagnosis_PathologicalDiagnosisController : BaseTopController
    {
        ...
        [Inject]
        public IMT_BC_PathologicalDiagnosisService mT_BC_PathologicalDiagnosisService { get; set; }
        ...
        public ActionResult List()
        {
            ...
            var list = mT_BC_PathologicalDiagnosisService.LoadSearchEntities(commonParam);
            totalCount = commonParam.TotalCount;
            int PageCount = Convert.ToInt32(Math.Ceiling((double)totalCount / pageSize));

            List<MT_BC_PathologicalDiagnosis> result = new List<MT_BC_PathologicalDiagnosis>();
            result.AddRange(list);
            PagerInfo pager = new PagerInfo();
            pager.PageIndex = pageIndex;
            pager.PageSize = pageSize;
            pager.TotalCount = totalCount;
            PagerQuery<PagerInfo, List<MT_BC_PathologicalDiagnosis>> query = new PagerQuery<PagerInfo, List<MT_BC_PathologicalDiagnosis>>(pager, result);
        ...
        //查看页
        public ActionResult PathologicalDiagnosisShow()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
        ...
        //修改页
        public ActionResult PathologicalDiagnosisUpdate()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
        ...
        public ActionResult Show()
        {
            ...
            if (id != "")
            {
                var result = mT_BC_PathologicalDiagnosisService.LoadEntityAsNoTracking(t => t.id.Contains(id));
