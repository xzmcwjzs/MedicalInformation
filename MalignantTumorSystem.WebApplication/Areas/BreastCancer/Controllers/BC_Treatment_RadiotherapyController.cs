...
{
    public class BC_Treatment_RadiotherapyController : BaseTopController
    {
        // GET: BreastCancer/BC_Treatment_Radiotherapy
        [Inject]
        ...
        [Inject]
        public IMT_BC_RadiotherapyService mT_BC_RadiotherapyService { get; set; }
        #region 框架页 
        ...
        public ActionResult AddAndUpdate()
        {
            ...
            MT_BC_Radiotherapy entity = new MT_BC_Radiotherapy();
            ...
            //------------------------------------特有字段赋值-------------------------------------------------------
            entity.fl = Request["fl"];
            entity.flqzd = Request["flqzd"];
            entity.bq = Request["bq"];
            entity.nljlxz = Request["nljlxz"];
            entity.fssyz = Request["fssyz"];
            entity.fsff = Request["fsff"];
            ...
            if (string.IsNullOrEmpty(id))
            {
                if (mT_BC_RadiotherapyService.AddEntity(entity))
                {
                    ...
                    ehr.item_type = Model.Enum.EHRAbstractTypeEnum.Radiotherapy.ToString();
            ...
            {
                if (mT_BC_RadiotherapyService.UpdateEntity(entity))
                {
                    msg = "修改成功";
                }
                else
        ...
        public ActionResult RadiotherapyShow()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
        ...
        public ActionResult RadiotherapyUpdate()
        {
            Comm_Platform_Worker workerModel = Session["worker"] as Comm_Platform_Worker;
            ViewBag.real_name = CommonFunc.SafeGetStringFromObj(workerModel.real_name);
            ViewBag.worker = CommonFunc.SafeGetStringFromObj(workerModel.user_name);
        ...
        public ActionResult Show()
        {
            ...
            if (id != "")
            {
                var result = mT_BC_RadiotherapyService.LoadEntityAsNoTracking(t => t.id.Contains(id));
