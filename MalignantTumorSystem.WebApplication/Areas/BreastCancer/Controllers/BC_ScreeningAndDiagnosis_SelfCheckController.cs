...
namespace MalignantTumorSystem.WebApplication.Areas.BreastCancer.Controllers
{
    public class BC_ScreeningAndDiagnosis_SelfCheckController : BaseTopController
    {
        ...
        [Inject]
        public IMT_BC_SelfCheckService selfCheckService { get; set; }
        #region 框架页
        ...
        public ActionResult List()
        {
            ...
            var list = selfCheckService.LoadSearchEntities(commonParam);
            totalCount = commonParam.TotalCount;
            int PageCount = Convert.ToInt32(Math.Ceiling((double)totalCount / pageSize));

            List<MT_BC_SelfCheck> result = new List<MT_BC_SelfCheck>();
            result.AddRange(list);
            PagerInfo pager = new PagerInfo();
            pager.PageIndex = pageIndex;
            pager.PageSize = pageSize;
            pager.TotalCount = totalCount;
            PagerQuery<PagerInfo, List<MT_BC_SelfCheck>> query = new PagerQuery<PagerInfo, List<MT_BC_SelfCheck>>(pager, result);
        ...
        public ActionResult AddAndUpdate()
        {
            ...
            MT_BC_SelfCheck entity = new MT_BC_SelfCheck();
            ...
            entity.tumor = Request["tumor"];
            entity.tumorbreast = Request["tumorbreast"];
            entity.thickening = Request["thickening"];
            entity.thickeningbreast = Request["thickeningbreast"];
            entity.symmetric = Request["symmetric"];
            entity.uplift = Request["uplift"];
            entity.skin = Request["skin"];
            entity.invagination = Request["invagination"];
            entity.discharge = Request["discharge"];
            ...
            if (string.IsNullOrEmpty(id))
            {
                if (selfCheckService.AddEntity(entity))
                {
                    ...
                    ehr.item_type = Model.Enum.EHRAbstractTypeEnum.SelfCheck.ToString();
            ...
            {
                if (selfCheckService.UpdateEntity(entity))
                {
                    msg = "修改成功";
                }
                else
        ...
        public ActionResult Show()
        {
            ...
            if (id != "")
            {
                var result = selfCheckService.LoadEntityAsNoTracking(t => t.id.Contains(id));
